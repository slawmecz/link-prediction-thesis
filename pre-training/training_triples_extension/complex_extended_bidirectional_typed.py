...
Adds both incoming and outgoing recommendations and the type of each entitiy.
Train with FB15k237_bidirectional_typed schema tree!!!
"""
...
def get_config(key, default=None):
    ...
    configs = {
        ...
        'model.max_epochs': 19,     
        'model.batch_size_train': 1024,  
        'model.batch_size_eval': 256,  
        'model.learning_rate': 0.18255429345236635,  
        'sampling.negative_o': 1000,  
        'model.dropout': 0.5,  
        'model.regularize_weight': 1.3408200403806817e-08,  
        ...
        'probability_threshold': 0.25,  # Probability threshold for recommendations
        'max_recommendations': 10,  #for each entitiy
...
def get_entity_outgoing_properties(triples: torch.Tensor, entity_id: int, id_to_relation: Dict[int, str], id_to_entity: Dict[int, str] = None, entity_types: Dict[str, str] = None) -> set:
    """
    ...
    Args:
        ...
        id_to_entity: Mapping from entity IDs to their labels (optional)
        entity_types: Mapping from entity names to their types (optional)
        
    Returns:
        Set of relation labels where the entity is the head with "O:" prefix and entity type
    """
    ...
    entity_type = "unknown"
    if id_to_entity and entity_types and entity_id in id_to_entity:
        entity_name = id_to_entity[entity_id]
        entity_type = entity_types.get(entity_name, "unknown")
    
    # Get relation labels with "O:" prefix and entity type
    return {f"O:{entity_type}:{id_to_relation[rel_id.item()]}" for rel_id in head_triples[:, 1]}

def get_entity_incoming_properties(triples: torch.Tensor, entity_id: int, id_to_relation: Dict[int, str], id_to_entity: Dict[int, str] = None, entity_types: Dict[str, str] = None) -> set:
    """
    ...
    Returns:
        Set of relation labels where the entity is the tail with "I:" prefix and entity type
    """
    ...
    # Get relation labels with "I:" prefix and entity type
    return {f"I:{entity_type}:{id_to_relation[rel_id.item()]}" for rel_id in tail_triples[:, 1]}
...
def get_recommendations(properties: List[str], api_url: str = None) -> List[Dict[str, Any]]:
    ...
    # Keep prefixes intact since the recommender was trained on data with prefixes
...
def create_artificial_triples(
    ...
    entity2type_path: str = "entity2type.txt",
    type_frequencies_path: str = "type_frequencies.txt"
    ...
    print("\n=== Creating Artificial Triples (Bidirectional with Types) ===")
    
    # Load entity types
    print("Loading entity types...")
    try:
        entity_types = load_entity_types(entity2type_path, type_frequencies_path)
        print(f"Loaded types for {len(entity_types)} entities")
    except Exception as e:
        print(f"Warning: Could not load entity types: {e}")
        print("Proceeding without entity types...")
        entity_types = {}
    ...
    # Create id_to_relation and id_to_entity mappings
    ...
    for entity_id in all_entities:
        ...
        outgoing_props = get_entity_outgoing_properties(triples, entity_id, id_to_relation, id_to_entity, entity_types)
        
        # Get incoming properties (entity as tail)
        incoming_props = get_entity_incoming_properties(triples, entity_id, id_to_relation, id_to_entity, entity_types)
    ...
    for entity_id, props in entity_properties.items():
        ...
        # Limit recommendations to the number of original properties (both incoming and outgoing) - skip if no needed
        ...
        for new_prop, probability in filtered_recommendations:
            ...
            if new_prop.startswith("I:"):
                is_incoming = True
                # New format: I:type:relation
                parts = new_prop.split(":", 2)  # Split into at most 3 parts
                if len(parts) >= 3:
                    entity_type = parts[1]
                    prop_name = parts[2]
                    print(f"Parsed typed incoming property: type={entity_type}, relation={prop_name}")
                else:
                    # Old format: I:relation
                    prop_name = new_prop[2:]  # Remove "I:" prefix
                    print(f"Parsed old incoming property: relation={prop_name}")
            elif new_prop.startswith("O:"):
                # New format: O:type:relation
                parts = new_prop.split(":", 2)  # Split into at most 3 parts
                if len(parts) >= 3:
                    entity_type = parts[1]
                    prop_name = parts[2]
                    print(f"Parsed typed outgoing property: type={entity_type}, relation={prop_name}")
                else:
                    # Old format: O:relation
                    prop_name = new_prop[2:]  # Remove "O:" prefix
                    print(f"Parsed old outgoing property: relation={prop_name}")
            else:
                prop_name = new_prop  # No prefix, use as is
                print(f"Parsed property without prefix: {prop_name}")
            ...
            #add type - either delete the type recommnedatoin or add a triple with with the type (but what would be the label? isType? there is no such label I think in my dataset)
...
def train_extended_model(
    ...
    new_triples, next_entity_id = create_artificial_triples(
        ...
        entity2type_path="entity2type.txt",
        type_frequencies_path="type_frequencies.txt"
    )
    ...
    # Setup PyKEEN pipeline parameters
    random_seed = get_config('model.random_seed')
    pipeline_kwargs = {
        'evaluator': get_config('model.evaluator', 'rankbased'),
        'evaluator_kwargs': {"batch_size": get_config('model.batch_size_eval')},
