...
Includes addition of only outgoing relations.
Train with FB15k237_outgoing schema tree!!!
"""
...
def get_recommendations(properties: List[str], api_url: str = None) -> List[Dict[str, Any]]:
    ...
    Args:
        properties: List of property names/IDs (e.g., "genre" for FB15k237 or "P101" for CoDEx)
...
def create_artificial_triples(
    ...
    """
    Create artificial triples based on recommendations.
    ...
    # Count unique head entities in training set
    unique_heads = set(triple[0].item() for triple in dataset.training.mapped_triples)
    print(f"Number of unique head entities in training set: {len(unique_heads)}")
    ...
    # Set next entity ID
    next_entity_id = len(dataset.entity_to_id)
    ...
    for triple in dataset.training.mapped_triples:
        ...
        # Only collect properties where entity is the head
    ...
    for head_id, properties in entity_properties.items():
        if triple_count >= max_new_triples:
            break
        
        # Get recommendations for all properties of this entity
        property_list = list(properties)
        print(f"\nGetting recommendations for entity {head_id}")
        print(f"Entity {head_id} has {len(property_list)} original properties")
        recommendations = get_recommendations(property_list)
        
        print(f"Before filtering: {len(recommendations)} recommendations")
        filtered_recommendations = process_recommendations(recommendations, threshold=probability_threshold)
        # Limit recommendations to the number of original properties
        filtered_recommendations = filtered_recommendations[:len(property_list)]
        print(f"After probability filtering: {len(filtered_recommendations)} recommendations")
         
        # Comment out if you don't want the limitation to allow more varied recommendations
        filtered_recommendations = filtered_recommendations[:len(property_list)]
        print(f"Using {len(filtered_recommendations)} recommendations for entity {head_id}")
        
        # See if we're getting unique recommendations or if they're the same across entities
        if len(filtered_recommendations) > 0:
            prop_ids = [p[0] for p in filtered_recommendations[:3]]
            prob_vals = [p[1] for p in filtered_recommendations[:3]]
            print(f"Sample properties: {prop_ids}")
            print(f"Sample probabilities: {prob_vals}")
        ...
        for new_prop, probability in filtered_recommendations:
            ...
            if is_fb15k:
                # For FB15k237, we expect full paths
                if not new_prop.startswith('/'):
                    print(f"Skipping non-path property {new_prop} for FB15k237 dataset")
                    continue
            else:
                # For CoDEx, we expect P-numbers
                if not new_prop.startswith('P'):
                    print(f"Skipping non-P-number property {new_prop} for CoDEx dataset")
                    continue
            
            # Get the numeric relation ID for the new property
            if new_prop not in relation_to_id:
                print(f"Property not in known relations, skipping: {new_prop}")
                continue
            
            new_relation_id = relation_to_id[new_prop]
            
            # Get or create entity ID for this property
            if new_prop not in property_to_entity_id:
                property_to_entity_id[new_prop] = next_entity_id
                next_entity_id += 1
            
            # Create new triple using the property-specific entity ID
            new_triple = torch.tensor([head_id, new_relation_id, property_to_entity_id[new_prop]])
...
def train_extended_model(
    ...
    """
    Train a model with extended triples from the recommender.
    ...
    """
    print("\n=== Training Extended ComplEx Model ===")
    ...
    with open(metrics_file, 'w') as f:
        ...
        f.write(f"Dataset: {dataset_name} + {len(sampled_triples)} artificial triples\n")
